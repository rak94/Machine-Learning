...
def get_neighbors(train_data, test_row, num_neighbors):
    ...
    distances.sort(key=lambda ed: ed[1])
    neighbors = list()
    for i in range(num_neighbors):
        neighbors.append(distances[i][0])
    return neighbors

# classify by predicting the neighbors
def prediction_classification(train_data, test_row, num_neighbors):
...
print("Predicted ", (dataset[0][-1], prediction))